...
function loadFigures(){
 GL.load('models/kimpoy-rig.glb',function(gl){var root=gl.scene,mesh=null;root.traverse(function(m){if(m.isSkinnedMesh)mesh=m});if(!mesh)return;
  ...
  var hats=Object.keys(HATM).map(function(k){return HATM[k]});FIG.hats=hats;DOG.children.forEach(function(ch){if(ch!==wrap&&ch!==FIG.dia&&hats.indexOf(ch)<0&&!ch.userData.dianna)ch.visible=false});
 },undefined,function(){});
 GL.load('models/dianna-rig.glb',function(gl){var root=gl.scene,mesh=null;root.traverse(function(m){if(m.isSkinnedMesh)mesh=m});if(!mesh)return;
  fitRoot(root,FIG.diaH,0,0);skinMat(mesh,.85);var wrap=new THREE.Group();wrap.position.set(0,FIG.diaY,FIG.diaZ);wrap.rotation.x=FIG.lean;wrap.add(root);DOG.add(wrap);FIG.dia=root;var D=girlRoles(root);FIG.D=D;
  ...
  bakePose([[pick(D.arms[0]||[],0),_AZ,D.arms[0]&&D.arms[0].right?-1.15:1.15],[pick(D.arms[1]||[],0),_AZ,D.arms[1]&&D.arms[1].right?-1.15:1.15],[pick(D.arms[0]||[],0),_AX,.55],[pick(D.arms[1]||[],0),_AX,.55],[pick(D.arms[0]||[],2),_AX,.4],[pick(D.arms[1]||[],2),_AX,.4],   // T-pose arms: drop to the sides (about the forward axis), then forward to hold the fur

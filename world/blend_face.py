...
argv=sys.argv[sys.argv.index('--')+1:]; IN,OUT,TEXOUT=argv[0],argv[1],argv[2]
...
render=bpy.data.images.load(M+'diana-render.png'); render.colorspace_settings.name='sRGB'
# face-core alignment: the mesh's front head box (front-facing verts, Z .80-.96) -> the render's face box (x 200-305, y 70-185)
fv=(n[:,2]>.80)&(n[:,2]<.96)&(n[:,1]<.45); hx0,hx1=np.percentile(n[fv,0],8),np.percentile(n[fv,0],92); hz0,hz1=.80,.96
print('HEADBOX x',round(hx0,3),round(hx1,3))
FB=(200/W,70/H,305/W,185/H)
u=m('ADD',m('MULTIPLY',m('DIVIDE',m('SUBTRACT',X,hx0),hx1-hx0),FB[2]-FB[0]),FB[0]); v=m('SUBTRACT',1-FB[1],m('MULTIPLY',m('DIVIDE',m('SUBTRACT',hz1,Z),hz1-hz0),FB[3]-FB[1]))
comb=node('ShaderNodeCombineXYZ'); ln.new(u,comb.inputs[0]); ln.new(v,comb.inputs[1]); rt=node('ShaderNodeTexImage',image=render,extension='EXTEND'); ln.new(comb.outputs[0],rt.inputs['Vector'])
# mask: front-facing head band (face), soft edges, only where the render has content
fF=m('MULTIPLY',m('SUBTRACT',NF,.45),2.5,clamp=True)
zlo=m('MULTIPLY',m('SUBTRACT',Z,.80),12.0,clamp=True); zhi=m('MULTIPLY',m('SUBTRACT',.97,Z),12.0,clamp=True)
xc=m('MULTIPLY',m('SUBTRACT',(hx1-hx0)*.42,m('ABSOLUTE',m('SUBTRACT',X,(hx0+hx1)/2))),10.0,clamp=True)
mask=m('MULTIPLY',m('MULTIPLY',fF,m('MULTIPLY',zlo,zhi)),m('MULTIPLY',xc,rt.outputs['Alpha']))
mix=node('ShaderNodeMix',data_type='RGBA'); ln.new(mask,mix.inputs[0]); ln.new(base.outputs['Color'],mix.inputs[6]); ln.new(rt.outputs['Color'],mix.inputs[7])
